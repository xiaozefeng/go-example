...
func (r *router) findMdls(root *node, segs []string) []Middleware {
	...
	// root 是一个特例，特殊处理一下
	if len(root.mdls) > 0 {
		result = append(result, root.mdls...)
	}
	for _, seg := range segs {
		...
		for _, cur := range q {
			...
			if cur.paramChild != nil {
				path := cur.path[:1]
				if path == seg {
					result = append(result, cur.paramChild.mdls...)
					children = append(children, cur.paramChild)
				}
			}
			// 处理通配符子节点
			if cur.starChild != nil {
				result = append(result, cur.starChild.mdls...)
				children = append(children, cur.starChild)
			}
			// 处理静态路由
			if len(cur.children) > 0 {
				n, ok := cur.children[seg]
				if ok {
					result = append(result, n.mdls...)
