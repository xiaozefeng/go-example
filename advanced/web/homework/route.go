	...
	for _, s := range segs {
		...
		if matchParam {
			if root.typ == nodeTypeParam {
				...
				subMatch := root.regExpr.FindStringSubmatch(s)
				if len(subMatch) > 1 {
					mi.addValue(root.paramName, subMatch[1])
				}
			}
		}
	}
...
func (n *node) childOrCreate(path string) *node {
	if strings.ContainsAny(path, `()`) {
		...
		n.regChild = &node{path: path, typ: nodeTypeReg, regExpr: reg, paramName: paramName}
		return n.regChild
	}
	if path == "*" {
		if n.paramChild != nil {
